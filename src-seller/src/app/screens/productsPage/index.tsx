...
const availableSizes = ["S", "M", "L", "XL", "XXL", "XXXL"] as const;
              ...
              <tbody>
                {filteredProducts.map((product) => (
                  <tr key={product.id}>
                    ...
                    <td>{product.type}</td>
                    <td className="mnshop-seller-products-page__price">
                      <strong>{formatKrw(product.discountPrice || product.price)}</strong>
                      {product.sale && (
                        <small>SALE · was {formatKrw(product.price)}</small>
                      )}
                    </td>
                    <td>{product.stock}</td>
                    <td>{product.sold}</td>
...
function ProductEditor({ product, onClose, onSaved }: { product: SellerProduct | null; onClose: () => void; onSaved: (product: SellerProduct, created: boolean) => void }) {
  ...
  const [sale, setSale] = useState(Boolean(product?.sale));
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!name.trim() || !Number.isFinite(parsedPrice) || !Number.isInteger(parsedStock) || colors.length === 0 || sizes.length === 0 || (!product && images.length === 0)) {
      setError("Name, price, stock, at least one color, one size, and one product image are required.");
      return;
    }
    if (
      sale &&
      ...
      setError("Sale price must be greater than 0 and lower than the original price.");
      return;
    }
    setSaving(true);
    setError("");
    try {
      const saved = product
        ? await sellerProductService.updateProduct(product.id, { productName: name.trim(), productDesc: description.trim(), productType: type, productStatus: status, productPrice: parsedPrice, productDiscountPrice: sale ? parsedDiscountPrice : undefined, productLeftCount: parsedStock, productColors: colors, productSizes: sizes, productSale: sale })
        : await sellerProductService.createProduct({ name: name.trim(), description: description.trim(), type, status, price: parsedPrice, discountPrice: sale ? parsedDiscountPrice : undefined, sale, stock: parsedStock, colors, sizes, images });
